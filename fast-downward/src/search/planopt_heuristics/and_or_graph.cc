...
void AndOrGraph::most_conservative_valuation() {
    ...
    */
    ...
    while (queue.size() > 0) {
        auto id = queue.front();
        queue.pop_front();
        nodes[id].forced_true = true;
        for (auto pred_id : nodes[id].predecessor_ids) {
            nodes[pred_id].num_forced_successors += 1;
            if (visited.find(pred_id) != visited.end())
                continue;
            if ((nodes[pred_id].type == NodeType::OR) 
                || (unsigned) nodes[pred_id].num_forced_successors == nodes[pred_id].successor_ids.size()) {
